...
async function searchUser() {
    ...
    if (!userId) {
        showError('Please enter a User ID or Email');
        return;
    }
    ...
    try {
        ...
        displayProfile(profile, userId); // Pass the userId that was searched for
        hideLoading();
...
// Add current user profile button
function addCurrentUserButton() {
    if (!isUserLoggedIn()) {
        return;
    }
    ...
    const userId = userIdFromStorage || 
                   (userData ? (userData.uid || userData.userId || userData.id || userData.email) : null);
    if (userId) {
        const searchDiv = document.querySelector('.profile-search');
        const currentUserBtn = document.createElement('button');
        currentUserBtn.textContent = 'Refresh My Profile';
        currentUserBtn.onclick = function() {
            document.getElementById('user-search').value = userId;
            searchUser();
        };
        currentUserBtn.style.cssText = `
            padding: 10px 20px;
            background: #28a745;
            color: white;
            border: none;
        ...
        `;
        currentUserBtn.onmouseover = function() { this.style.background = '#218838'; };
        currentUserBtn.onmouseout = function() { this.style.background = '#28a745'; };
        
        searchDiv.appendChild(currentUserBtn);
    }
}

// Load current user profile automatically
function loadCurrentUserProfile() {
    // Check if user is logged in using existing utility function
    if (!isUserLoggedIn()) {
        return false;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check if user ID is passed in URL, otherwise load current user
